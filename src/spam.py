...
async def set(ctx):
    if ctx.channel.name == 'instructor-commands':
        ...
        msg_warn_num = int((await BOT.wait_for('message', timeout=60.0)).content)
        await ctx.send('How many messages should a user be able to send before they are put in timeout?')
        msg_timeout_num = int((await BOT.wait_for('message', timeout=60.0)).content)
        await ctx.send(f'How much time should users have to send {msg_warn_num} messages before they will be warned '
                       f'and then eventually put in timeout?')
        msg_time_clears = int((await BOT.wait_for('message', timeout=60.0)).content)
        await ctx.send('How long should a user be placed in timeout when caught spamming? Format: min,hours,days Example: 5,0,0 is 5 minutes')
        msg_timeout_time = (await BOT.wait_for('message', timeout=60.0)).content
